...
wait_for_glusterfs_cluster() {
    ...
    n=0
    until [ `kubectl get storagecluster gluster -o json | jq -r '.status.ready == true'` = "true" ] ; do
        n=$[$n+1]
        if [ $n -gt 600 ] ; then
...
teardown() {
    ...
    wait_for_pods_deleted "default" "gluster"
    wait_for_pods_deleted "default" "heketi"
}
